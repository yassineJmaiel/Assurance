        ...
        <div class="col-md-12">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb pl-0">
                    <li class="breadcrumb-item active" aria-current="page">Liste Demande remboursement</li>
                </ol>
            </nav>
            <div class="ms-panel">
                <div class="ms-panel-body">
                    <div class="table-responsive">
                        <table id="exemple2" class="table table-striped thead-primary w-100">
                            <thead>
                                <tr>
                                    <th>ID</th>
                                    <th> Assuré </th>
                            ...
                            <tbody>
                                @foreach($remboursement as $remboursementItem)
                                    <tr>
                                        ...
                                        <td> {{$remboursementItem->assurer->name}}
                                    ...
                                    @endif
                                        ...
                                        <td>
                                            @if($remboursementItem->piece_jointe<>null)
                             <button style="border-color:white" onclick="displayPDF('{{ $remboursementItem->piece_jointe }}')">
                                        ...
                                        @endif
                                        @if($remboursementItem->status=="en cours"  )
                                    
                                        <button type="button" style="margin-top: 1px" class="btn btn-primary repondre-btn" data-id="{{ $remboursementItem->id }}" data-assurer="{{ $remboursementItem->assurer_id }}" data-toggle="modal" data-target="#reponse">Repondre</button>
                                 @endif
...
<div class="modal fade" id="reponse" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog ms-modal-dialog-width modal-dialog-centered">
        <div class="modal-content ms-modal-content-width">
            <div class="modal-header bg-primary text-white ms-modal-header-radius-0">
                ...
                <div class="col-xl-12 col-md-12">
                    <div class="card">
                        <div class="card-header">
                            <h6 class="m-0">Repondre</h6>
                        </div>
                        <div class="card-body">
                            <form class="form" action="reponse" method="POST" >
                                @csrf
                                
                                <input type="hidden" id="remboursement_id" name="remboursement_id" >
                                <input type="hidden" id="assurer_id" name="assurer_id">
                                <div class="col-md-3">
                                <div class="form-group">
                                    <input type="radio" name="status" value="Accepté" class="form-check-input" id="accepte">
                                ...
                                <div class="form-group">
                                    <input type="radio" name="status" value="Refusé" class="form-check-input" id="refuse">
                                    <label for="refuse" class="form-check-label">Refusé</label>
                                </div>
                                </div>
                                <div class="form-group">
...
@endsection
<script>
    function displayPDF(piece_jointe) {
        var pdfUrl = '/uploads/remboursement/' + piece_jointe;
        window.open(pdfUrl, '_blank');
    }
</script>
<script>
    document.addEventListener('DOMContentLoaded', function() {
        var repondreBtns = document.querySelectorAll('.repondre-btn');
        
        repondreBtns.forEach(function(btn) {
            btn.addEventListener('click', function() {
                var remboursementId = this.getAttribute('data-id');
                var assurerId = this.getAttribute('data-assurer');
                
                document.getElementById('remboursement_id').value = remboursementId;
                document.getElementById('assurer_id').value = assurerId;
            });
        });
    });
</script>
